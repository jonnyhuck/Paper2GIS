...
* If you are using the OSM tiles option, you can add an optional **hillshade layer** courtesy of ESRI. An example of a Paper2GIS map with and without hillshade is given below:

![Hillshade Example](resources/images/hillshade.png)

* If you are using the OSM tiles, you also have the option of adding a boundary to your dataset (i.e., to mark the extent of your are of interest). This is achieved by providing a path to a Shapefile containing the boundary as a polygon. It will be drawn as an outline, with the width, colour and opacity all editable using the options. An example of a Paper2GIS map with a Shapefile boundary is given below (blue boundary around the national park):

  ![Hillshade Example](resources/images/boundary.png)
